...
- **页码偏移调整：** 通过全局“偏移量”输入框及 +/- 按钮，轻松校准纸质页码与 PDF 实际页码之间的偏差。
- **实时页面预览：** 在映射表中点击预览按钮（眼睛图标），即可即时查看对应 PDF 页面，确保切割位置准确无误。
...
3. **步骤 3: 审核与编辑：** 程序会自动计算初始偏移量。你可以通过 **+/- 按钮** 手动微调全局偏移，并利用每行末尾的 **预览按钮** 实时查看 PDF 页面，确保映射完全正确。
